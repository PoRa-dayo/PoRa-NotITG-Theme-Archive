...
function DWIITGReceptor()
	...
	t.LoadSelections = function(self, list, pn) if Profile().DWIITGReceptor==nil then list[1] = true elseif Profile().DWIITGReceptor then list[1] = true else list[2] = true end end
	t.SaveSelections = function(self, list, pn)
...
function SaveToProfile()
	...
	if GAMESTATE:IsPlayerEnabled(PLAYER_1) then
		...
		if not GAMESTATE:IsCourseMode() then
        	...
        	Profile().DWITotalStepsHitP1 = (Profile().DWITotalStepsHitP1 or 0) + StatsCombined(PLAYER_1, 8, 7, 6)

        	Profile().DWITotalSongsPlayedP1 = (Profile().DWITotalSongsPlayedP1 or 0) + 1

        	if GetScore(PLAYER_1) > (Profile().DWIHighestScoreP1 or 0) then Profile().DWIHighestScoreP1 = GetScore(PLAYER_1) end

        	if GetTotalScore(PLAYER_1) > (Profile().DWIHighestSessionScoreP1 or 0) then Profile().DWIHighestSessionScoreP1 = GetTotalScore(PLAYER_1) end

        	if GetPSStageStats(PLAYER_1):MaxCombo() > (Profile().DWIHighestComboP1 or 0) then Profile().DWIHighestComboP1 = GetPSStageStats(PLAYER_1):MaxCombo() end
			...
			if GameSecs > (Profile().DWILongestTimeNonstopP1 or 0) then Profile().DWILongestTimeNonstopP1 = GameSecs end

			if GetPSStageStats(PLAYER_1):MaxCombo() > (Profile().DWIHighestComboNonstopP1 or 0) then Profile().DWIHighestComboNonstopP1 = GetPSStageStats(PLAYER_1):MaxCombo() end

			if GetScore(PLAYER_1) > (Profile().DWIHighestScoreNonstopP1 or 0) then Profile().DWIHighestScoreNonstopP1 = GetScore(PLAYER_1) end
    ...
    if GAMESTATE:IsPlayerEnabled(PLAYER_2) then
    	...
    	if not GAMESTATE:IsCourseMode() then
    		...
    		Profile().DWITotalStepsHitP2 = (Profile().DWITotalStepsHitP2 or 0) + StatsCombined(PLAYER_2, 8, 7, 6)

        	Profile().DWITotalSongsPlayedP2 = (Profile().DWITotalSongsPlayedP2 or 0) + 1

        	if GetScore(PLAYER_2) > (Profile().DWIHighestScoreP2 or 0) then Profile().DWIHighestScoreP2 = GetScore(PLAYER_2) end

        	if GetTotalScore(PLAYER_2) > (Profile().DWIHighestSessionScoreP2 or 0) then Profile().DWIHighestSessionScoreP2 = GetTotalScore(PLAYER_2) end

        	if GetPSStageStats(PLAYER_2):MaxCombo() > (Profile().DWIHighestComboP2 or 0) then Profile().DWIHighestComboP2 = GetPSStageStats(PLAYER_2):MaxCombo() end
			...
			if GameSecs > (Profile().DWILongestTimeNonstopP2 or 0) then Profile().DWILongestTimeNonstopP2 = GameSecs end

			if GetPSStageStats(PLAYER_2):MaxCombo() > (Profile().DWIHighestComboNonstopP2 or 0) then Profile().DWIHighestComboNonstopP2 = GetPSStageStats(PLAYER_2):MaxCombo() end

			if GetScore(PLAYER_2) > (Profile().DWIHighestScoreNonstopP2 or 0) then Profile().DWIHighestScoreNonstopP2 = GetScore(PLAYER_2) end
