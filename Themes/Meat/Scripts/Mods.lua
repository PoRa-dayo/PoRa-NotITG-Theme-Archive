...
function SM(input) local str = tostring(input); if type(input) == "table" then str = ""; for i,v in ipairs(input) do if i>1 then str = str .. ", " end str = str..tostring(v) end end SCREENMAN:SystemMessage(str) end
...
--  q is the index of the last entry of difficultyList. We need to save this instead of using table.getn because when you have "FixedDifficultyRow" you often have nil values in the middle of the table.
	for n=1,2 do if Player(n) then
		for i=1,q do if difficultyList[i] == GAMESTATE:GetCurrentSteps(n-1) then listPointer[n] = i; break end end
